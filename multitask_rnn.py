...
class MultitaskRNN(nn.Module):
    ...
    def forward(self, x, x_lengths,h_state ):
        ...
        loan_done_outs = []
        reward_outs=[]
        for time_step in range(paded_out.size(1)):    # calculate output for each time step
            state_outs.append(self.state_layer(paded_out[:, time_step, :]))
            installment_done_outs.append(self.installment_done_layer(paded_out[:, time_step, :]))
