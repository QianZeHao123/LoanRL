...
class MultitaskLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, varstate_size):
        ...
        self.loan_done_layer = nn.Sequential(nn.Linear(hidden_size, 1), nn.Sigmoid())
        self.reward_layer = nn.Sequential(nn.Linear(hidden_size, 1), nn.ReLU())
    ...
    def forward(self, x, x_lengths, h_state):
        ...
        for time_step in range(
            ...
            loan_done_outs.append(self.loan_done_layer(paded_out[:, time_step, :]))
            reward_outs.append(self.reward_layer(paded_out[:, time_step, :]))
        return (
            torch.stack(state_outs, dim=1),
            torch.stack(installment_done_outs, dim=1),
            torch.stack(loan_done_outs, dim=1),
            torch.stack(reward_outs, dim=1),
            h_state,
        )
